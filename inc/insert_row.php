...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  $sql = "SELECT fallotment_id FROM `tbl_genfund` WHERE `reference_id` = '$reference_id'";
  $result = $conn->query($sql);
  $row = $result->fetch_array();
  $fallotment_id = $row['fallotment_id'];


  $fetch = "SELECT genfund_year FROM `tbl_genfund` WHERE `reference_id` = '$reference_id'";
  $result = $conn->query($fetch);
  $row = $result->fetch_array();
  $genfund_year = $row['genfund_year'];
  ...
  $sql = "INSERT INTO `tbl_genfund`(`reference_id`,`fallotment_id`,`genfund_year`, `prog_type`)
          VALUES ('$reference_id','$fallotment_id','$genfund_year','$prog_type')";
