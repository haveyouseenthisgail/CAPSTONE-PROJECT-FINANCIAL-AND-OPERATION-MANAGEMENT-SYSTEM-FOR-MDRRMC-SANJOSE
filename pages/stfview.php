  ...
  <div class="wrapper">
    ...
    <div class="content-wrapper">
      ...
      if (isset($_GET['reference_id'])) {
        ...
        $query = mysqli_query($conn, "SELECT * FROM `tbl_stf` WHERE `reference_id` = '$reference_id' ");
        $result = mysqli_fetch_array($query);
        extract($result);
      }

      $sql = "SELECT * FROM tbl_stf ORDER BY reference_id ASC";
      ...
      $query = mysqli_query($conn, "SELECT SUM(exp_mooe) AS total_exp_mooe, SUM(first_quart) AS total_first_quart, SUM(second_quart) AS total_second_quart, SUM(third_quart) AS total_third_quart, SUM(fourth_quart) AS total_fourth_quart, SUM(remarks) AS total_remarks  FROM `tbl_stf` WHERE `reference_id` = '$reference_id'");
      if ($query) {
        $result = mysqli_fetch_array($query);
        $totalExpMooe = $result['total_exp_mooe'];
        $totalFirstQuart = $result['total_first_quart'];
        $totalSecondQuart = $result['total_second_quart'];
        $totalThirdQuart = $result['total_third_quart'];
        $totalFourthQuart = $result['total_fourth_quart'];
      ...
      <form action="../inc/controller.php" method="POST" id="stfForm">
                          ...
                          <table border='0' cellpadding='0' cellspacing='0' width='1190' style='border-collapse:collapse;
                            ...
                            <tr height='20' style='mso-height-source:userset;height:15pt'>
                              ...
                              <td colspan='9' class='x46'>
                                ...
                                <?php echo date('Y', strtotime($stf_year)) ?> ANNUAL INVESTMENT PROGRAM (AIP)
                              </td>
                              <td colspan='2' class='x22' style='mso-ignore:colspan;'></td>
                            </tr>
                            <tr height='20' style='mso-height-source:userset;height:15pt'>
                        ...
                        <body link='blue' vlink='purple'>
                          ...
                          <table border='0' cellpadding='0' cellspacing='0' width='1007' style='border-collapse:collapse;
                            ...
                            <div class="prevmt__row" id="prevMt">
                              ...
                              $query = mysqli_query($conn, "SELECT * FROM `tbl_stf` WHERE `prog_type` = 'prevention'");
                              $counter = 1;
                              ...
                              ?>
                                <tr height='54' style='mso-height-source:userset;height:40.5pt' id="prevMtRow">
                                  ...
                                  <td class='x31' style="padding-left:6px;">
                                    <textarea name="program1[]" id="" style="height: 51px;width:148px;border:1px solid #fff;outline:0;font-size:9pt;resize:none;" readonly> <?php echo $program ?> </textarea>

                                  </td>
                                  <td class='x31'>
                                    <textarea name="office1[]" id="" style="display:flex;resize:none;height: 51px;width:85px;border:1px solid #fff;outline:0;text-align:center;" readonly> <?php echo $office ?>  </textarea>
                                  </td>
                                  <td class='x29'>
                                    <textarea class="ex__outputs" name="exp_outputs1[]" id="textInput" style="display: flex; height: 51px;width: 175px;resize: none;border: 1px solid #fff;outline:0;" readonly><?php echo $exp_outputs ?> </textarea>
                                  </td>
                                  <td class='x32'>
                                    <input type="number" name="exp_mooe1[]" style="    height: 50px; width: 87px;font-size: 9pt;border:1px solid #fff;outline:0;text-align:center;" value="<?php echo $exp_mooe ?>" readonly>
                                  </td>
                                  <td class='x38' style="align-items: center;height: 5rem;padding-left: 8px; padding-bottom:15px;">
                                    <input type="number" name="first_quart1[]" style="height: 51px;width:
																100px;outline:0;border:1px solid #fff;text-align:center;" value="<?php echo $first_quart ?>" readonly>
                                  </td>
                                  <td class='x38' style="">
                                    <div class="flex flex-justify--center flex-align--center" style="height: 5rem;">
                                      <input type="number" name="second_quart1[]" style="height: 51px; width: 100px;border: 1px solid #fff;outline: 0;text-align:center;" value="<?php echo $second_quart ?>" readonly>
                                    </div>
                                  </td>
                                  <td class='x38'>
                                    <div class="flex flex-justify--center flex-align--center" style="height: 5rem;">
                                      <input type="number" name="third_quart1[]" style="height: 51px;width: 105px;border: 1px solid #fff;outline: 0;text-align: center;" value="<?php echo $third_quart ?>" readonly>
                                    </div>
                                  </td>
                                  <td class='x38'>
                                    <div class="flex flex-justify--center flex-align--center" style="height:5rem">
                                      <input type="number" name="fourth_quart1[]" style="height: 51px;width: 100px;border: 1px solid #fff;outline: 0;text-align: center;" value="<?php echo $fourth_quart ?>" readonly>
                                    </div>
                                  </td>
                                  <td class="x38">
                                    <div class="flex flex-justify--center flex-align--center" style="height:5rem">
                            ...
                            <div class="preparedness_row">
                              <?php
                              $conn = new mysqli('localhost', 'root', '', 'mdrsystem');
                              $query = mysqli_query($conn, "SELECT * FROM `tbl_stf` WHERE `prog_type`= 'preparedness' ");
                              while ($result = mysqli_fetch_array($query)) {
                                extract($result);
                              ?>
                            ...
                            <div class="support__row">
                              <?php
                              $conn = new mysqli('localhost', 'root', '', 'mdrsystem');
                              $query = mysqli_query($conn, "SELECT * FROM `tbl_stf` WHERE `prog_type`= 'support' ");
                              while ($result = mysqli_fetch_array($query)) {
                                extract($result);
                            ...
                            <div class="response__row">
                              <?php
                              $conn = new mysqli('localhost', 'root', '', 'mdrsystem');
                              $query = mysqli_query($conn, "SELECT * FROM `tbl_stf` WHERE `prog_type`= 'response' ");
                              while ($result = mysqli_fetch_array($query)) {
                                extract($result);
                              ?>
                                <tr height='67' style='mso-height-source:userset;height:50.25pt' id="respRow">
                                  ...
                                  <td class='x31' style="height: 5rem;padding-left: 8px;padding-top: 4px;">
                                    <textarea name="program4[]" id="" style="resize:none;border:1px solid #fff;outline:0;height:72px;width:144px;" readonly>
                                  <?php echo $program ?>
                                </textarea>
                                  </td>
                                  <td class='x31'>
